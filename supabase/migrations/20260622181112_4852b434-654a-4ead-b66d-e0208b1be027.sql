...
BEGIN
  ...
  SELECT auth_user_id, email, nome INTO v_auth_user_id, v_email, v_nome
  ...
  DELETE FROM public.contatos WHERE v_email IS NOT NULL AND lower(email) = lower(v_email);
  GET DIAGNOSTICS v_count = ROW_COUNT;
  v_counts := v_counts || jsonb_build_object('contatos_by_email', v_count);
  ...
  WHERE v_email IS NOT NULL AND lower(solicitante_email) = lower(v_email);
  GET DIAGNOSTICS v_count = ROW_COUNT;
  v_counts := v_counts || jsonb_build_object('tickets_solicitante_by_email', v_count);
  ...
  SELECT
    ...
    (SELECT count(*) FROM public.tickets WHERE v_email IS NOT NULL AND lower(solicitante_email) = lower(v_email)) +
    (SELECT count(*) FROM public.contatos WHERE v_email IS NOT NULL AND lower(email) = lower(v_email)) +
